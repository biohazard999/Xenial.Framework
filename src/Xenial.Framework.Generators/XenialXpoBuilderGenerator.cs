...
public class XenialXpoBuilderGenerator : IXenialSourceGenerator
{
    ...
    )
    {
        ...
        //    var @attribute = GetXenialViewIdsAttribute(@classSymbol, generateXenialXpoBuilderAttribute);
    ...
    )
    {
        ...
        if (isAttributeDeclared && !@class.HasModifier(SyntaxKind.PartialKeyword))
        {
            context.ReportDiagnostic(
                Diagnostic.Create(
                    GeneratorDiagnostics.ClassNeedsToBePartial(xenialXpoBuilderAttributeFullName),
                    @class.GetLocation()
            ));
