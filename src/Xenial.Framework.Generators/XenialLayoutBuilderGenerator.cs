...
public class XenialLayoutBuilderGenerator : IXenialSourceGenerator
{
    ...
    )
    {
        ...
        foreach (var @class in types)
        {
            ...
            {
                var layoutBuilderAttribute = targetType.FindAttribute(layoutBuilderAttributeSymbol);

                if (layoutBuilderAttribute is not null && layoutBuilderAttribute.ConstructorArguments.Length > 0)
                {
                    var firstArgument = layoutBuilderAttribute.ConstructorArguments.First();

                    if (firstArgument.Kind == TypedConstantKind.Type && firstArgument.Value?.ToString() == @classSymbol.ToString())
                    {
                        var builderName = "BuildLayout";

                        if (layoutBuilderAttribute.ConstructorArguments.Length >= 2)
                        {
                            var builderOverload = layoutBuilderAttribute.ConstructorArguments[1];
                            if (builderOverload.Kind == TypedConstantKind.Primitive)
                            {
                                builderName = builderOverload.Value?.ToString();
                            }
                        }

                        //TODO: warning if not marked as partial
                        if (!@classSymbol.MemberNames.Contains(builderName))
                        {
                            ...
                            using (builder.OpenBrace($"namespace {@classSymbol.ContainingNamespace}"))
                            {
                                ...
                                using (builder.OpenBrace($"partial {(@classSymbol.IsRecord ? "record" : "class")} {@classSymbol.Name}"))
                                {
                                    builder.WriteLine($"public partial Layout {builderName}();");
                                }
                            }

                            compilation = AddGeneratedCode(context, compilation, @class, builder, addedSourceFiles, hintName: "partial");
                        }
                    }
                }
            }
