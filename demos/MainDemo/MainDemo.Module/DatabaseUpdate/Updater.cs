...
namespace MainDemo.Module.DatabaseUpdate
{
    public class Updater : ModuleUpdater
    {
        ...
        public override void UpdateDatabaseAfterUpdateSchema()
        {
            ...
            var topTenEmployees = ObjectSpace.GetObjects<Employee>();
            ...
            for (var i = 0; i < topTenEmployees.Count; i++)
            {
                var employee = topTenEmployees[i];
                ...
                var resume = ObjectSpace.FirstOrDefault<Resume>(r => r.Employee == employee);
                if (resume == null)
                {
                    ...
                    try
                    {
                        var stream = FindEmployeeResume(employee);
                        if (stream != null)
                        {
                ...
                var reviewerEmployee = i < 5 ? FindTellitson() : FindJanete();
                var note = ObjectSpace.FirstOrDefault<Note>(n => n.Text.Contains(employee.FullName));
                if (note == null)
                {
...
#if !EASYTEST
                var appointment = ObjectSpace.FirstOrDefault<Event>(a => a.Subject.Contains(employee.FullName));
                if (appointment == null)
                {
                    ...
                    var reviewerEmployeeResource = ObjectSpace.FirstOrDefault<Resource>(r => r.Caption.Contains(reviewerEmployee.FullName));
            ...
            var taskList = GenerateTask(employees);
            if (taskList.Count > 0)
            {
                ...
                foreach (var employee in employees)
                {
                    ...
                    {
                        var index = rndGenerator.Next(1, taskList.Count - 2);
            ...
            var user1 = ObjectSpace.FirstOrDefault<ApplicationUser>(u => u.UserName == "Sam");
            if (user1 == null)
            {
            ...
            var userRole = ObjectSpace.FirstOrDefault<PermissionPolicyRole>(role => role.Name == "Users");
            if (userRole == null)
            {
        ...
        {
            var taskAnalysis1 = ObjectSpace.FirstOrDefault<Analysis>(a => a.Name == "Completed tasks");
            ...
            var taskAnalysis2 = ObjectSpace.FirstOrDefault<Analysis>(a => a.Name == "Estimated and actual work comparison");
        ...
        private void UpdateAnalysisCriteriaColumn()
        {
            ...
            {
                var length = (int)ExecuteScalarCommand(@"select CHARACTER_MAXIMUM_LENGTH from INFORMATION_SCHEMA.Columns WHERE TABLE_NAME = 'Analysis' AND COLUMN_NAME = 'Criteria'", true);
                if (length != -1)
                {
        ...
        {
            var departmentsTable = GetDepartmentsDataTable();
            var employeesTable = GetEmployeesDataTable();
            IList<Department> result = new List<Department>();
            foreach (DataRow departmentData in departmentsTable.Rows)
            {
                var departmentTitle = Convert.ToString(departmentData["Title"]);
                var department = FindDepartment(departmentTitle);
                if (department == null)
                {
                    ...
                    var departmentHeadId = Convert.ToString(departmentData["DepartmentHead"]);
                    var employee = employeesTable.Select($"Id = '{departmentHeadId}'").FirstOrDefault();
                    if (employee != null)
                    {
        ...
        {
            var email = Convert.ToString(employeeRow["EmailAddress"]);
            var employee = ObjectSpace.FirstOrDefault<Employee>(e => e.Email == email, true);
            if (employee == null)
            {
                ...
                var titleOfCourtesyText = Convert.ToString(employeeRow["Title"]).ToLower();
                ...
                var countryName = Convert.ToString(employeeRow["CountryRegionName"]);
                var country = ObjectSpace.FirstOrDefault<Country>(c => c.Name == countryName, true);
                if (country == null)
                {
                ...
                var departmentTitle = Convert.ToString(employeeRow["GroupName"]);
                employee.Department = FindDepartment(departmentTitle);

                var positionTitle = Convert.ToString(employeeRow["JobTitle"]);
                var position = FindPosition(positionTitle);
                if (position == null)
                {
        ...
        {
            var rndGenerator = new Random();
            var taskList = new List<DemoTask>();
            var tasksTable = GetTasksDataTable();
            foreach (DataRow taskRow in tasksTable.Rows)
            {
                var taskSubject = UpdateContent(Convert.ToString(taskRow["Subject"]));
                if (ObjectSpace.FirstOrDefault<DemoTask>(t => t.Subject == taskSubject) == null)
                {
                    ...
                    var rndStatus = rndGenerator.Next(0, 5);
        ...
        {
            var shortName = employee.FirstName + "_" + employee.LastName + ".pdf";
            var embeddedResourceName = Array.Find<string>(GetType().Assembly.GetManifestResourceNames(), (s) => { return s.Contains(shortName); });
            return GetType().Assembly.GetManifestResourceStream(embeddedResourceName);
        }
        private bool LocationIsEmpty(Employee employee) => employee.Location == null || employee.Location.Employee == null || (employee.Location.Latitude == 0 && employee.Location.Longitude == 0);
        private void InitEmployeeLocation()
        {
            var employeeMary = FindTellitson();
            ...
            var employeeJohn = FindEmployee("John", "Nilsen");
            ...
            var employeeKarl = FindEmployee("Karl", "Jablonski");
    ...
    public abstract class TaskAnalysis1LayoutUpdaterBase
    {
        ...
        public void Update(Analysis analysis)
        {
            ...
            {
                var control = CreateAnalysisControl();
...
#endif
    internal class PayrollSampleDataGenerator
    {
