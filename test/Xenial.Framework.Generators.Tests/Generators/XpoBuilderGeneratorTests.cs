...
[UsesVerify]
public class XpoBuilderGeneratorTests : PartialGeneratorTest<XenialXpoBuilderGenerator>
{
    protected override XenialXpoBuilderGenerator CreateTargetGenerator() => new();

    public Task RunSourceTest(string fileName, string source, Func<IEnumerable<PortableExecutableReference>>? additionalReferences = null, [CallerFilePath] string filePath = "")
        => RunTest(o => o with
        {
            SyntaxTrees = o => new[]
            {
            ...
            }).Concat(additionalReferences?.Invoke() ?? Enumerable.Empty<PortableExecutableReference>()),
            Compile = false
        }, filePath);
